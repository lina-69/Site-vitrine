...
class TemoignageBlock(blocks.StructBlock):
    """
    Bloc "témoignage" pour StreamField.
    On pourra en ajouter plusieurs dans l'admin Wagtail.
    """
    ...
    auteur = blocks.CharBlock(
        ...
        help_text="Nom de la personne qui témoigne"
    )

    fonction = blocks.CharBlock(
        required=False,
        max_length=120,
        label="Fonction / rôle",
        help_text="Fonction ou rôle de l'auteur du témoignage"
    )

    photo = ImageChooserBlock(
        required=False,
        help_text="Photo de l'auteur (format portrait recommandé).",
        label="Photo de l'auteur"
    )

    message = blocks.TextBlock(
        required=True,
        label="Message du témoignage",
        help_text="Le contenu du témoignage"
    )
    ...
    rating = blocks.IntegerBlock(
        ...
        max_value=5,
        label="Note",
        help_text="Note sur 5 étoiles (optionnel)"
    )

    class Meta:
        icon = "user"
        label = "témoignage"
        help_text = "Bloc réutilisable pour présenter un témoignage avec photo et message."
